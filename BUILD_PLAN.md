...
- `npm run docs:screenshots` regenerates every image against a reset fixture at a fixed viewport, and product/stock/BOM content is fully deterministic run-to-run. **Caveat found while building this (2026-07-31), recorded rather than hidden:** manufacture-order/purchase-order ids and "Created" timestamps are real, live values rendered on screen, so the handful of screenshots that show them (Manufacturing, Purchasing, the Ledger tab) are *not* byte-identical across regenerations even though the underlying data is — the fixture cleanup step resets those tables' AUTO_INCREMENT so ids at least start from the same baseline each run, but the live timestamp genuinely can't be frozen without adding a test-only clock seam this doesn't warrant. A "zero git diff" claim across the whole set would be false; verified instead by confirming re-running twice never changes *which* screenshots exist, their content shape, or anything other than these two live-value classes.
...
### Phase 8 — In-app documentation & training module (~3–4 days, added 2026-07-30, re-estimated 2026-07-31) — ✅ **done and verified 2026-07-31, see CLAUDE.md Progress Log**
- Per §5.12: `Admin\GuidePage` (plain PHP, one small `Section` value object per topic under `src/Admin/Guide/Sections/`, aggregated by `GuideContent`), WP-native contextual help tabs on all six screens (five core + Purchasing when enabled) via `Admin\Guide\ContextualHelp`, generated screenshots via a dev-only Playwright script (`npm run docs:screenshots`, `bin/capture-docs-screenshots.mjs` + two eval-file fixture scripts) that runs with the vendors feature turned on and strips any third-party admin notice before every capture, a coverage test (`tests/GuideCoverageTest.php`) that fails when a page/route/CLI command has no doc section, and the one third-party companion-plugin video **linked, never embedded**.
- **Ordering rule (the developer's, 2026-07-30):** documentation is built last so nothing is created after it and left out of training. Built after Phases 7, 9, and 10, as planned.
- ✅ Demo (verified 2026-07-31): a new user can follow the Guide alone from fresh install → working made-to-order product → completed manufacture order → stock receipt; a merchant can find, enable, and use Vendors & Purchase Orders from the Guide alone; the coverage test fails when a section's `covers_pages`/`covers_routes`/`covers_commands` is edited to drop real coverage, and passes with 21 real screenshots + all 32 registered REST routes + all 4 CLI subcommands documented; the Guide page loads zero external resources (checked via the browser network panel — every request stayed on `localhost`); a contextual help tab renders correctly on Manufacturing with a working deep link into the Guide; the built release zip contains `assets/docs/` and `assets/css/`.
...
### Phase 11 — Order-time cost snapshot & profitability reports (~2–3 days, added 2026-08-16) — ✅ **done and verified 2026-08-16, see CLAUDE.md Progress Log**
...
- ✅ Demo (verified 2026-08-16): placed a made-to-order sale, confirmed a `bom_live`-sourced row captured matching that moment's live BOM cost; completed a manufacture order and sold one of the finished units, confirmed a `mo_snapshot`-sourced row; changed the component's price afterward and re-ran the report — the original sale's cost and profit stayed unchanged (the drift rule); CSV export byte-matched the on-screen numbers; a real bug was found and fixed along the way — `wc_get_orders()` without an explicit `type` silently include `WC_Order_Refund` objects, fataling report generation the moment any order had ever been refunded (see CLAUDE.md Progress Log for the fix).
