...
**State at end of session:** the developer is installing the correct (arm64) Docker Desktop himself. Still pending: (1) confirm the right-architecture Docker Desktop is installed, (2) launch `/Applications/Docker.app` and complete first-run setup (accept license, grant privileged-helper permission — macOS will prompt for password directly), (3) verify with `docker info` that the daemon is up and `docker info | grep -i arch` (or equivalent) shows arm64/aarch64.
