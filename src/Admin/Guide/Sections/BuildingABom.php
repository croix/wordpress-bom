...
final class BuildingABom {
	...
	public static function get(): Section {
		return new Section(
			id: 'building-a-bom',
			title: __( 'Building a BOM', 'pv-bom-stock' ),
			blocks: array(
				Section::text(
					'<p>' . __( 'Open any product for editing and use its "Bill of Materials" tab to add lines. Each line picks a component (searched by name, or created inline if it does not exist yet) and a quantity. A live "buildable" count and cost estimate update as you edit.', 'pv-bom-stock' ) . '</p>'
						. '<p>' . __( 'Each line has a condition:', 'pv-bom-stock' ) . '</p>'
						. '<ul>'
						. '<li>' . __( '<strong>Always</strong> — consumed on every sale, regardless of what the customer chose. The blank tumbler, the epoxy, the standard cap.', 'pv-bom-stock' ) . '</li>'
						. '<li>' . __( '<strong>Attribute</strong> — only consumed when a specific variation attribute is selected (e.g. glitter color = Blue). Pick the attribute and value from dropdowns populated from the product\'s own variations.', 'pv-bom-stock' ) . '</li>'
						. '<li>' . __( '<strong>Add-on</strong> — only consumed when a specific Extra Product Options field has a specific value (e.g. "Add Stickers" = Yes).', 'pv-bom-stock' ) . '</li>'
						. '</ul>'
