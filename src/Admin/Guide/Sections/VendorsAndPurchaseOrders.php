...
final class VendorsAndPurchaseOrders {
	...
	public static function get(): Section {
		return new Section(
			...
			title: __( 'Vendors & Purchase Orders', 'pv-bom-stock' ),
			blocks: array(
				Section::text(
					'<p>' . __( '<strong>This entire feature is optional and off by default.</strong> If you order supplies informally and just receive them on the Component Inventory screen, there is nothing here you need — skip this section.', 'pv-bom-stock' ) . '</p>'
						. '<p>' . __( 'Turn it on with "Enable vendors & purchase orders" on the Settings page. A new "Purchasing" menu item appears (nothing else changes for anyone not using it), starting empty — turning it on never creates sample vendors or orders. It has a Vendors tab (name, contact, notes)…', 'pv-bom-stock' ) . '</p>'
				),
				Section::screenshot(
					'purchasing-vendors.png',
					__( 'The Purchasing screen\'s Vendors tab.', 'pv-bom-stock' )
				),
				Section::text(
				...
				Section::screenshot(
					...
					__( 'The Purchasing screen\'s Purchase Orders tab, filterable by status.', 'pv-bom-stock' )
				),
				Section::screenshot(
					'purchasing-new-po.png',
					__( 'The new Purchase Order modal with a vendor picker and component lines.', 'pv-bom-stock' )
				),
				Section::text(
					'<p>' . __( 'Freight, tax, and other fees can be entered on a purchase order at any time (even after it is fully received, since the real bill often arrives later) and are spread across the order\'s lines in proportion to what each line cost, giving a landed cost per component. This is for your own visibility only — it is never written back into a product\'s price or fed into the margin/COGS reports.', 'pv-bom-stock' ) . '</p>'
				),
				Section::screenshot(
					'purchasing-receive-costs.png',
					__( 'The Receive modal and the freight/tax/fees cost fields on a purchase order.', 'pv-bom-stock' )
				),
				Section::text(
					'<p>' . __( 'A placed order can be emailed with "Send PO" — to the vendor, to yourself, or both. Recipients always come from the vendor\'s own record and your own account, never a typed-in address.', 'pv-bom-stock' ) . '</p>'
						. '<p>' . __( 'Once enabled, an "On order" figure appears on the Component Inventory screen and in the Low Stock report — a low component with a shipment already on the way is still shown as low (hiding it would be misleading), just with a note of what is expected and when.', 'pv-bom-stock' ) . '</p>'
				),
			),
			links: array(
				array(
					'label' => __( 'Open Purchasing', 'pv-bom-stock' ),
					'url'   => admin_url( 'admin.php?page=wcbom-purchasing' ),
				),
				array(
					'label' => __( 'Enable it on Settings', 'pv-bom-stock' ),
