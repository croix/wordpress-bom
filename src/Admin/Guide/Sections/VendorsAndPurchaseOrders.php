...
final class VendorsAndPurchaseOrders {
	...
	public static function get(): Section {
		return new Section(
			...
			body: '<p>' . __( '<strong>This entire feature is optional and off by default.</strong> If you order supplies informally and just receive them on the Component Inventory screen, there is nothing here you need — skip this section.', 'wcbom' ) . '</p>'
				. '<p>' . __( 'Turn it on with "Enable vendors & purchase orders" on the Settings page. A new "Purchasing" menu item appears (nothing else changes for anyone not using it), starting empty — turning it on never creates sample vendors or orders.', 'wcbom' ) . '</p>'
				. '<p>' . __( 'The Purchasing screen has a Vendors tab (name, contact, notes) and a Purchase Orders tab. A purchase order moves through a small lifecycle: <strong>draft</strong> (freely editable, deletable), <strong>ordered</strong> (placed with the vendor — lines lock; to correct a mistake, cancel and start a new draft), then <strong>partially received</strong> or <strong>received</strong> as deliveries arrive. Receiving more than was ordered is allowed and simply flagged, never blocked — vendors do ship extra sometimes. "Cancel" is offered at any point before fully received, and relabels itself to "Close" once something has already arrived — both end an order that will not be completed further; whatever already arrived stays received either way.', 'wcbom' ) . '</p>'
			...
			screenshots: array(
				array(
					'file' => 'purchasing-vendors.png',
					'alt'  => __( 'The Purchasing screen\'s Vendors tab.', 'wcbom' ),
				),
				array(
					'file' => 'purchasing-po-list.png',
					'alt'  => __( 'The Purchasing screen\'s Purchase Orders tab, filterable by status.', 'wcbom' ),
				),
				array(
					'file' => 'purchasing-new-po.png',
					'alt'  => __( 'The new Purchase Order modal with a vendor picker and component lines.', 'wcbom' ),
				),
				array(
					'file' => 'purchasing-receive-costs.png',
					'alt'  => __( 'The Receive modal and the freight/tax/fees cost fields on a purchase order.', 'wcbom' ),
				),
			),
			links: array(
				array(
